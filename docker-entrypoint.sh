...
set -e
#устанавливает параметр оболочки для немедленного выхода,
#если какая-либо выполняемая команда завершается с ненулевым кодом выхода.
...
if [ "$1" = 'consul' -a -z "${CONSUL_DISABLE_PERM_MGMT+x}" ]; then
  ...
  #moiseev
  CONSUL_USER=$(id -nu ${CONSUL_UID})
  CONSUL_GROUP=$(getent group ${CONSUL_GID} | awk -F: '{print $1}')
  set -- sudo -u ${CONSUL_USER} -g ${CONSUL_GROUP}  "$@"
fi

exec "$@"
#заменит текущую запущенную оболочку командой, "$@"на которую указывает.
